...
@pytest.mark.asyncio
async def test_rebuild_status_template_is_allowed(monkeypatch: pytest.MonkeyPatch) -> None:
    captured: dict[str, tuple[str, ...]] = {}
    ...
    await run_storcli(
        ["/c0/e2/s0", "show", "rebuild", "J"],
        use_sudo=False,
        binary_path="storcli64",
    )

    assert captured["argv"] == ("storcli64", "/c0/e2/s0", "show", "rebuild", "J")
