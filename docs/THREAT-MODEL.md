...
| Denial of service | Local user pre-creates the collector or metrics lock file with their own uid to keep the daemon from acquiring it. | Default `collector_lock_path` and `metrics_lock_path` now live under `/var/lib/megaraid-dashboard` (`0750`, service-owned), which no other local user can write into; the existing `_validate_process_lock_file` ownership/symlink/regular-file checks remain in place as defence in depth. | Operators who override the defaults to a world-writable directory reintroduce the risk. |
...
3. Collector and metrics lock files now default to `/var/lib/megaraid-dashboard/*.lock`,
   removing the `/tmp` DoS pre-creation vector. The notifier lock still defaults to `/tmp`
